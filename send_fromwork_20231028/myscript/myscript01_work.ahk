...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    #IfWinActive,ahk_exe EXCEL.exe
        ...
        ^+Left::Send  ^+{Left}
        ^+Right::Send ^+{Right}
        +WheelUp:: ComObjActive("Excel.Application").ActiveWindow.SmallScroll(0,0,0,1)
        +WheelDown:: ComObjActive("Excel.Application").ActiveWindow.SmallScroll(0,0,1,0)
        ^Tab:: send !h6
        ^+Tab:: send !h5
        ^2:: send, !hfdo
        ^3:: send, !hh{Right}{Right}{Right}{Right}{Right}{Right}{Right}{Down}{Down}{Enter}
        ^+3:: send, !hhn
        ^4:: send, !hfc{Down}{Down}{Down}{Down}{Down}{Down}{Down}{Right}{Enter}
        ^+4:: send, !hfc{Enter}
        sc07B & Right:: send, {Blind}{End}
        sc07B & left:: send, {Blind}{Home}
        sc07B & WheelUp::ComObjActive("Excel.Application").ActiveWindow.SmallScroll(0,1,0,0)
        sc07B & WheelDown::ComObjActive("Excel.Application").ActiveWindow.SmallScroll(1,0,0,0)
        ; ^WheelUp::
        ;     exl_zoom := exl_zoom + 5
        ;     ComObjActive("Excel.Application").ActiveWindow.Zoom := exl_zoom
        ;     return
        ; ^WheelDown::
        ;     exl_zoom := exl_zoom - 5
        ;     ComObjActive("Excel.Application").ActiveWindow.Zoom := exl_zoom
...
#IfWinActive, ahk_class SunAwtFrame
    ...
    ;;; mouse wheel works as zoom in/out ;;;
    ; this works correctly, but this is not confortable to use. i dont use it
    ; wheelup::
    ;     send, !{up}
    ;     return
    ; wheeldown::
...
;;; draw io ;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; because letsnote's process speed is low, 
; wheel function cant be run correctly
; this is not cause of ahk code (i have checked to run only ahk code of freeplane part, but it dont work correctly)
